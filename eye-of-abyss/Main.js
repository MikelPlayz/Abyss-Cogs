...
async function loginWithRetry(client, token, retries = 5, delay = 5000) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.error(`Login attempt ${attempt} failed. Retrying in ${delay / 1000}s...`, err);
...
(async () => {
    await loginWithRetry(discordBot, config.get().discord.token);
    if (start) {
