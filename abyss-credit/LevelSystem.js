...
class LevelSystem {
    ...
    findTotalXp(id) {
        if(this.findLevel(id)<41) {
		return Math.ceil(330 * Math.log10(Util_1.Util.factorial(this.findLevel(id) + 2)));
    	}
	else {
		return Math.ceil(40000 * Math.pow(4, this.findLevel(id) - 40));
	}
    }
    findLevel(id) {
        const xp = this.findXp(id);
        for (let level = 100; level >= 41; level--) {
    ...
    levelUp(id, level) {
        ...
        Main_1.discordClient.sendEmbed(Embeds_1.Embeds.levelUp(user, level, rewards), Main_1.config.get().leveling.channel).then();
    }
}
